    ...
    get "get_messages_comments_by_id" do
      ...
      if m
        ...
        @comments.each do |com|
          res[:comments].merge!("id-#{com.id}" => {user: {nickname: com.user.nickname, avatar: com.user.avatar.url.gsub("public","") + "?time=#{com.user.updated_at.to_i}"},id: "id-#{com.id}",content: com.body, liked: com.liked_by_user?(u), published_at: com.created_at.strftime( "%Y-%m-%d %H:%M")})
        end
        return res
      else
